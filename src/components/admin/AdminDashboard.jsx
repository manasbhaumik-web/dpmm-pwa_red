...
import { DocPreview, DocModal, EditMemberModal } from './SharedModals';
        ...
        {open && (
          <div className="border-t border-slate-100 p-4 pb-8 bg-slate-50/30 space-y-4 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-2">
              ...
              <button className="w-full sm:w-auto justify-center bg-slate-100 hover:bg-slate-200 text-slate-650 text-xs font-bold px-4 py-1.5 rounded-xl transition-all duration-200 active:scale-95 border border-slate-200" style={{ minHeight: '36px' }}>
                Reject Application
              </button>
            </div>
          </div>
...
function AddMemberModal({ onClose, onSave, adminType, adminState }) {
  ...
  const [stateLoc, setStateLoc] = useState(adminType === 'state' ? adminState : 'Selangor');
...
export default function AdminDashboard({ onToast, adminType, adminState }) {
  const [activeTab, setActiveTab] = useState('dashboard');
  const [viewTarget, setViewTarget] = useState(null);

  const getFilteredPending = () => adminType === 'state' ? dbInstance.pendingQueue.filter(p => p.state === adminState) : dbInstance.pendingQueue;
  const getFilteredMembers = () => dbInstance.members
    .filter(m => adminType === 'state' ? m.state === adminState : true)
    .map(m => ({
      id: m.member_id,
  ...
  const getFilteredPayments = () => {
    if (adminType === 'state') {
      const allowedIds = dbInstance.members.filter(m => m.state === adminState).map(m => m.member_id);
      return dbInstance.payments.filter(p => allowedIds.includes(p.member_id));
    }
    return [...dbInstance.payments];
  };

  const [pending, setPending] = useState(getFilteredPending());
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('All');
  
  // Re-read from relational dbInstance dynamically to allow updates
  const [membersList, setMembersList] = useState(getFilteredMembers());

  // Relational payments database state
  const [paymentsList, setPaymentsList] = useState(getFilteredPayments());
  ...
  const refreshLocalState = () => {
    setPending(getFilteredPending());
    setMembersList(getFilteredMembers());
    setPaymentsList(getFilteredPayments());
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {showAddModal && <AddMemberModal onClose={() => setShowAddModal(false)} onSave={handleAddNewMember} adminType={adminType} adminState={adminState} />}
            ...
            <div className="xl:col-span-3 space-y-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-1.5 text-xs text-slate-700 placeholder-slate-450 focus:outline-none focus:border-slate-900 transition-colors"
                />
              </div>
            ...
            <div className="card p-5 bg-slate-900 border border-slate-800 text-white space-y-4 border-t-2 border-t-blue-900">
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="w-full bg-slate-800 hover:bg-slate-900 disabled:opacity-40 text-white font-bold py-1.5 rounded-xl text-xs uppercase tracking-wider transition-all duration-200 active:scale-95 flex items-center justify-center gap-1.5 shadow-md shadow-slate-900/10"
                  style={{ minHeight: '40px' }}
                >
                  {webhookProcessing ? (
                    <>
          ...
          <div className="card p-4 grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white border border-slate-200 border-t-2 border-t-blue-900">
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-slate-900"
              >
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden border border-slate-200">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${
                          catName === 'Ordinary' ? 'bg-slate-900' : catName === 'Associate' ? 'bg-accent/90' : 'bg-emerald-500'
                        }`}
